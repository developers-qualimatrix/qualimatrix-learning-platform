    ...
    return (
        <>
                ...
                <div className="qt-modal-bg">
                    ...
                    <div className={`qt-form-wrap ${theme}-theme`}>
                        <div className={`qt-form ${theme}-theme`}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "auto auto auto", gap: "16px" }} className="grid grid-cols-1 md:grid-cols-3 gap-4 my-5">
                                {weekData.map((entry: any, index) => (
                                    <div
                                        style={{ width: "300px" }}
                                        key={index}
                                        className={`p-4 border rounded-lg shadow-sm cursor-pointer transition duration-300 ${selectedSchedule === entry.date
                                            ? "border-2 border-blue-500 bg-blue-100"
                                            : "bg-gray-100 hover:border-blue-400"
                                            }`}
                                        onClick={() => handleScheduleSelect(entry.date)}
                                    >
                                        <p className="text-lg font-medium text-gray-900">{entry.date}</p>
                            ...
                            <div className="w-100 d-md-flex justify-content-center mt-5">
                                <button className="" onClick={handleSubmit} >Register Now</button>
            ...
            {selectedSchedule && modalShow && (
                <RegisterAccountForm selectedSchedule={selectedSchedule} show={modalShow} onHide={() => setModalShow(false)} />
            )}
        </>
    );
};
